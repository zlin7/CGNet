...
//cum is cum_tauOut_m, tauMids is just tauMid
...
//Take m and m2, instead of m1 and m2, so as to save some space
            ...
            for (int m1 = -l1; m1 <= l1; m1++){
                for (int m2 = -l2; m2 <= l2; m2++){
                    ...
                    if (-l <= m && m <= l){
                        ...
                        //float CGcoef = stupidCGcal(l1,l2,l,m,m2,logfact);
                        for (int t1 = 0; t1 < taus[l1]; t1++){
                            for (int t2 = 0; t2 < taus[l2]; t2++){
                                int t = t1 * taus[l2] + t2 + t_offset;
                                float real1 = tensor[IDX(b,l1,t1,m1+l1,0,cum_taus,Lmax)];
                                float imag1 = tensor[IDX(b,l1,t1,m1+l1,1,cum_taus,Lmax)];
                                float real2 = tensor[IDX(b,l2,t2,m2+l2,0,cum_taus,Lmax)];
                                float imag2 = tensor[IDX(b,l2,t2,m2+l2,1,cum_taus,Lmax)];
                                out_tensor[IDX(b,l,t,m+l,0,cum_new_taus,Lmax)] += (real1 * real2 - imag1 * imag2) * CGcoef;
                                out_tensor[IDX(b,l,t,m+l,1,cum_new_taus,Lmax)] += (real1 * imag2 + real2 * imag1) * CGcoef;
...
void cudaUpBatchNorm_gpu_forward_kernel(
    ...
    float* middle = THCudaTensor_data(state, middle_tensor);

    int* tauIn = THIntTensor_data(tauIn_tensor);
    ...
    int* tauOut = THIntTensor_data(tauOut_tensor);
    ...
    cudaCG_forward_job<<<cuda_gridsize(size1), BLOCK, 0, stream>>>(
            F, middle,
    ...
    float* moving_std = THCudaTensor_data(state, moving_std_tensor);
    cudaBatchNorm_forward_job<<<cuda_gridsize(size2), BLOCK, 0, stream>>>(
            middle, moving_std,
    ...
    float* weights = THCudaTensor_data(state, weight_tensor);

    int size3 = B * cum_tauOut_m[L+1];
    //printf("Step 3 Need %d threads \n", size3);
    cudaWeightTransform_forward_job<<<cuda_gridsize(size3), BLOCK, 0, stream>>>(
            middle, weights,out,
            cuda_tauMiddle, cuda_cum_tauMiddle_m, 
...
void cudaUpBatchNorm_gpu_backward_kernel(
    ...
    cudaStream_t stream = THCState_getCurrentStream(state);
    //First, update the grad on weights
    float* Fin = THCudaTensor_data(state, input_tensor);
    ...
    float* grad_out = THCudaTensor_data(state, grad_out_tensor);
    float* grad_weight = THCudaTensor_data(state, grad_weight_tensor);
    ...
    float* grad_middle = THCudaTensor_data(state, grad_middle_tensor);

    int size1a = B * (L+1) * (L+2) * (L+1) /2;
    
    //printf("Step 1.a Need %d threads\n", size1a);
    gettimeofday(&t1, NULL);
    cudaCG_forward_job<<<cuda_gridsize(size1a), BLOCK, 0, stream>>>(
            Fin, grad_middle,
            cuda_tauIn, cuda_cum_tauIn_m, cuda_cum_tauMiddle_m,
            cuda_logfact, L, B);
    cudaThreadSynchronize();
    ...
    float* moving_std = THCudaTensor_data(state, moving_std_tensor);
    gettimeofday(&t1, NULL);
    cudaBatchNorm_forward_job<<<cuda_gridsize(size1b), BLOCK, 0, stream>>>(
    ...
    gettimeofday(&t1, NULL);
    cudaWeightGrad1_backward_job<<<cuda_gridsize(size1c), BLOCK, 0, stream>>>(
    ...
    gettimeofday(&t1, NULL);
    cudaMiddleGrad_backward_job<<<cuda_gridsize(size2), BLOCK, 0, stream>>>(
    ...
    float* grad_in = THCudaTensor_data(state, grad_in_tensor);
    int size3 = B * cum_tauIn_m[L+1];
    //printf("Step 3 Need %d threads\n", size3);

    gettimeofday(&t1, NULL);
    cudaCG_backward_job<<<cuda_gridsize(size3), BLOCK, 0, stream>>>(
    ...
    elapsedTime = (t2.tv_sec - t1.tv_sec) * 1000.0;      // sec to ms
    elapsedTime += (t2.tv_usec - t1.tv_usec) / 1000.0;   // us to ms
    //printf("free memory: %lf mili-seconds\n", elapsedTime);
}
